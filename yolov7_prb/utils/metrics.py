...
class ConfusionMatrix:
    ...
    def plot(self, save_dir='', names=()):
        try:
            ...
            sn.heatmap(array, annot=True, annot_kws={"size": 14}, cmap='Blues', fmt='.0f', square=True,
            ...
            np.savetxt(Path(save_dir) / 'confusion_matrix.csv', self.matrix, delimiter=',', fmt='%d')
            # Save the matrix as NumPy file
            np.save(Path(save_dir) / 'confusion_matrix.npy', self.matrix)

        except Exception as e:
            print(f'Error while plotting the confusion matrix: {e}')
            pass
